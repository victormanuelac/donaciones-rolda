...
`docs/02-Modelo-Datos-MER-DDL.md` (12 tablas de M1-6: `geographic_zones`, `organizations`, `users`, `warehouses`, `categories`, `master_items`, `stock_entries`, `stock_exits`, `audit_logs`, `internal_notifications`, `warehouse_assignments`, `expiry_alerts`) y `docs/11-Modulo-7-...md` (8 tablas de M7: `beneficiaries`, `vulnerability_scores`, `protocol_recommendations`, `beneficiary_recommendations`, `care_history`, `health_referrals`, `alerts`, `statistics_cache`) **no están fusionados en un único DDL, y ninguno de los dos describe ya el esquema real**. En código: `families`/`census_entries`/`beneficiaries` sí existen (censo Fase 1, ver abajo) pero con columnas distintas a las que documenta cada uno; `protocol_recommendations`/`beneficiary_recommendations` (Fase 2) también existen ya, con un esquema recortado del que propone el doc 11 (ver sección de Módulo 7 más abajo). `vulnerability_scores`, `care_history`, `health_referrals`, `alerts`, `statistics_cache` siguen sin implementarse. Si implementas migraciones nuevas, verifica el esquema real con `php artisan model:show` o leyendo la migración, no asumas que el documento manda.

**Inconsistencia conocida — resuelta en código, no en el documento:** el DDL de `02-Modelo-Datos-MER-DDL.md` (documento de diseño) todavía muestra `users.role ENUM('admin', 'acopio_operator', 'field_leader')` (3 roles) y no se ha corregido ahí. En la app real ya está resuelto: `app/Enums/UserRole.php` (backed enum, cast en `User::casts()`) define los 6 roles vigentes (`admin/operator/coordinator/doctor/donor/municipal`), con columna `role` (+ `status` vía `App\Enums\UserStatus`) agregada a `users` en `database/migrations/2026_08_18_220710_add_role_and_status_to_users_table.php`. RBAC por middleware (`role:` en las rutas), panel de aprobación de usuarios y las Policies por recurso (`UserPolicy`, `WarehousePolicy`, `MasterItemPolicy`, `BeneficiaryPolicy`, etc.) ya están implementados.

**Campos PII/salud a encriptar a nivel de aplicación (AES-256, cast `encrypted`/`encrypted:array`):** `users.document_id`, `users.phone`, `users.email`, `warehouses.contact_phone`, `stock_exits.received_by_name`, `families.head_document_number`, `families.phone`, `beneficiaries.document_number`, `beneficiaries.medical_notes`, `beneficiaries.chronic_conditions`, `beneficiaries.current_symptoms`. Trátalo como requisito desde el primer commit que toque estas tablas, no como mejora futura.
...
### Kardex (Módulo 3): mejoras evaluadas, pendientes de aplicar

Además de FEFO, bloqueo de despacho vencido, traslados entre bodegas, alertas de vencimiento/stock mínimo, proyección de agotamiento (`App\Services\Kardex\StockProjectionService`), ficha Kardex por ítem (`App\Services\Kardex\KardexLedgerService`, ruta `/kardex/ficha`) y conteo físico con ajustes (`App\Actions\Kardex\RegisterStockCountAction`, ruta `/kardex/conteo`) ya implementados, quedan evaluadas pero **sin implementar** dos mejoras adicionales — impleméntalas ahí si se retoman, no las trates como si ya existieran:

- **Doble verificación (maker-checker) en salidas grandes:** que una salida/entrega por encima de cierto umbral de cantidad requiera confirmación de un segundo rol (coordinador/admin) antes de descontar el inventario. Hoy cualquier operador con la bodega asignada puede despachar cualquier cantidad sin aprobación adicional — es un control anti-fraude estándar en programas de donación, no implementado.
- **Reportes exportables (CSV/Excel) de movimientos:** catalogado como parte del Módulo 5 (Alertas y Auditoría) en `docs/14-Modulos-y-Funcionalidades.md`, nunca implementado. Sin esto, cualquier reporte para donantes/autoridades se arma manualmente contra la base de datos.

### Módulo 7, Fase 2: perfil de vulnerabilidad individual y recomendaciones — implementado, con alcance recortado

Submódulo aparte del censo Fase 1 (rutas `/beneficiarios/...`, no `/censo/nuevo`) — decisión explícita: la Fase 1 es triaje ultrarrápido puerta a puerta y no debía cargarse con ~10 campos clínicos/sociales por persona. La Fase 2 la completan coordinador/admin/doctor (`User::canManageBeneficiaries()`, `BeneficiaryPolicy`) sobre hogares ya triados, extendiendo con columnas nuevas (nullable) la misma tabla `beneficiaries` de la Fase 1 — no hay una tabla paralela.

Score ponderado 0-100 sobre 4 factores — demográfico (0-30), salud (0-30), nutricional (0-20), social (0-20) — que determina 3 niveles de prioridad: ≥70 `critical`, 40-69 `priority`, <40 `normal` (`App\Enums\BeneficiaryPriorityLevel`). Ver `App\Services\Beneficiaries\VulnerabilityScoringService`. Es un sistema de puntaje **independiente** del `VulnerabilityIndexService` de la Fase 1 (`App\Services\CensusScoring`, 5 dimensiones/4 niveles/por hogar) — no se intenta reconciliarlos, son cosas distintas (triaje de hogar vs. perfil clínico de persona). Si se modifica la lógica de puntaje de Fase 2, hazlo ahí y en `docs/11-Modulo-7-...md` a la vez.

El motor de recomendaciones (`App\Services\Beneficiaries\BeneficiaryRecommendationService`) cruza el perfil contra `protocol_recommendations` vigentes (rango de edad / embarazo / condición crónica) y genera `beneficiary_recommendations` con una foto del stock disponible y la bodega más cercana (`App\Services\Geo\HaversineDistance` contra `families.latitude/longitude`). Los 3 protocolos sembrados (`database/seeders/BeneficiariesDemoSeeder.php`) son los ejemplos ilustrativos del doc 11 (embarazo/OMS, menores de 5/ICBF, diabetes/salud local) — **no es una librería médica curada por un especialista**, es solo demo/prueba; si se usa en producción, esa librería la debe curar alguien con criterio clínico, no fabricarla un agente.

**Deliberadamente sin implementar todavía** (documentado para no asumir que ya existe): tabla de histórico `vulnerability_scores` (hoy el score vive solo en `beneficiaries.vulnerability_score` + `last_score_update`, se sobrescribe cada vez, sin versiones anteriores), `care_history`/`health_referrals` (seguimiento de entregas y derivaciones a salud externa), `alerts` (sistema de alertas automáticas), `statistics_cache` + caché Redis, 6 dashboards por rol, integración M1 (demanda agregada anónima en el buscador público), integración M3 (observer de brecha stock-vs-demanda sobre `StockEntry`), integración M6 (que confirmar una entrega marque `FULFILLED` la recomendación y notifique al donante — hoy M6 y M7 no están conectados entre sí).
...
- **CI/CD:** GitHub Actions — `.github/workflows/tests.yml` corre en PRs y push a `main`/`test` (Pint, Pest, PHPStan). **`deploy.yml` todavía no existe**: el doc 18 §7.7 lo especifica (automático a `test`, manual con aprobación a producción) pero el archivo no está creado porque depende de que exista la configuración de OIDC/IAM en AWS. Los pasos del despliegue sí están en `deploy.sh`, en la raíz. El repo es **público**, así que no se usan runners autoalojados: el acceso a las instancias es por **AWS SSM con OIDC**, sin secretos de larga vida ni puertos de entrada — ver doc 18 §7.2 para el razonamiento.

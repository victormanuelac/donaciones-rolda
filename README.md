...
- **`main` y `test` están protegidas en GitHub** (ruleset, Settings → Rules) — acotado a esas dos ramas específicamente, no a todo el repositorio (las ramas `feature/*`/`fix/*`/`docs/*` quedan libres para hacer `amend`/rebase/force-push mientras trabajas, sin perder protección donde importa). Reglas activas en `main`/`test`:
  1. **Solo vía Pull Request** — push directo rechazado salvo el owner del repo (que tiene bypass; no lo uses para saltarte el flujo, es solo para emergencias). **Aprobación requerida: 0 por ahora** — GitHub no permite que el autor de un PR apruebe su propio PR, así que con un solo colaborador (hoy) exigir 1 aprobación deja todo bloqueado sin salida. Súbela a 1 en Settings → Rules → Rulesets en cuanto se sume un segundo colaborador con rol `Write` — ese es el momento en que la regla empieza a aportar de verdad.

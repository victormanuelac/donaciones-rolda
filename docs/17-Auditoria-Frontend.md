...
1. **XSS almacenado en el portal público** — el popup del mapa interpola nombres de ítem/bodega sin escapar. Vector de ejecución en el navegador de ciudadanos anónimos.
2. **El ambiente EC2 corre sobre HTTP plano** → sin *secure context*: el Service Worker no se registra, `crypto.randomUUID()` no existe y **los formularios de campo se cuelgan al enviar** sin mensaje de error.
3. **Falta la regla `[x-cloak]{display:none}`** — 28 elementos que deberían estar ocultos son visibles en el primer pintado de 8 vistas.
...
6. **El dashboard post-login es un placeholder** que muestra `0` en todos sus indicadores y el texto "todavía no hay módulos operativos", contradiciendo la base de datos.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Portal público** (`/`) | ⚠️ **Fallo silencioso de red**: si `fetch` lanza, no hay `catch` → muestra "No encontramos insumos" cuando el problema real es la conexión | 🔴 **XSS**: `item_name`/`warehouse_name` sin escapar en el popup de Leaflet (en las tarjetas sí van con `x-text`) | ⚠️ Sin `AbortController`: respuestas fuera de orden pisan resultados nuevos | ✅ Buena (`grid-cols-1 lg:grid-cols-3`); ⚠️ `autofocus` abre el teclado al entrar | ➖ N/A por diseño (requiere red) |
| **Login / Registro / Auth** | ✅ OK | ✅ OK | ✅ Ligero | ✅ OK | ➖ N/A |
| **Dashboard** | 🔴 **Placeholder puro**: indicadores en `0` hardcodeados, texto "no hay módulos operativos" | 🔴 **Cero conexión con la BD**. Es la primera pantalla de los 6 roles | ✅ Trivial | ✅ OK | ➖ |
...
| **Kardex — entrada / salida / traslado** | 🔴 **Botón atascado en "Guardando…"** si no hay *secure context*; ⚠️ "Solicitar ítem nuevo" (Livewire) muere en silencio sin red, en una pantalla rotulada "funciona sin conexión" | ⚠️ `quantity` viaja como cadena; la guarda `!quantity` no bloquea `"0"` | ✅ Ligero | ✅ `max-w-2xl` correcto | ✅ **Lo mejor de la app**: cola Dexie + reintento; 🔴 payload sin cifrar |
| **Kardex — ficha / conteo / vencimientos** | ✅ OK | ✅ OK | ⚠️ `availableQuantity()` por fila | 🔴 Tabla recortada (`overflow-hidden`) | ➖ |
| **Censo (wizard 6 pasos)** | 🔴 Mismo cuelgue de `crypto.randomUUID`; ✅ fallback GPS → pin manual bien resuelto | ⚠️ Payload plano, sin validación de cliente entre pasos | ⚠️ **Fuga de memoria**: el mapa Leaflet nunca se destruye con `wire:navigate` | ✅ Diseñado para móvil | ✅ Cola + `syncNow()`; 🔴 **documentos y teléfonos en claro en IndexedDB** |
...
### 🔴 [Crítica] C-1 · XSS almacenado en el portal público — **[verificado]**
...
La asimetría lo confirma: las mismas cadenas **sí** se escapan en las tarjetas de resultados (`x-text`); únicamente el popup del mapa es vulnerable.

### 🔴 [Crítica] C-2 · Sin *secure context*, la capa offline no existe y los formularios se cuelgan — **[verificado]**
...
### 🔴 [Crítica] C-3 · Falta la regla `[x-cloak]` — **[verificado]**
...
Efecto en **8 vistas / 28 elementos**: al entrar a `/kardex/entradas` el usuario ve simultáneamente el panel de éxito *"Entrada registrada"*, el formulario, el callout de error vacío y el texto *"Guardando…"* — todo junto — hasta que Alpine arranca y los oculta. En el portal público destellan a la vez "Buscando…", "No encontramos insumos" y el botón de ubicación. Es CLS puro y confusión directa para el usuario.
...
### 🔴 [Alta] A-5 · El dashboard contradice la base de datos — **[verificado]**

`resources/views/dashboard.blade.php` tiene `0` literal y hardcodeado en las tres tarjetas de indicadores ("Módulos activos", "Bodegas registradas", "Beneficiarios censados") y el texto *"Todavía no hay módulos operativos"* — con 6 módulos construidos, bodegas sembradas y hogares censados presentes en la BD. Es la primera pantalla que ve cualquiera de los 6 roles tras iniciar sesión.
...
### Bloque 0 — Antes que cualquier otra cosa (~1 día)
...
### Bloque 1 — Rendimiento y memoria (~2 días)
...
### Bloque 2 — Offline e IndexedDB (~3 días)
...
### Bloque 3 — UI/UX y accesibilidad (~2 días)
...
|---|---|---|---|
| 1 | Bloque 0 | ~1 día | C-1, C-2 (parcial), C-3, A-5 |
...
El Bloque 0 es el único urgente: cierra el XSS del portal público, que es el único hallazgo con exposición a usuarios anónimos.
